...
{
    if(m_clickedPoint.isNull() && m_view->board()->data(field.y(), field.x()) != ' ')
	{
        m_clickedPoint = field;
        m_selectedField = new ChessView::FieldHightlight(field.y(), field.x(), QColor(255, 0, 0, 50));
        m_view->addHighlight(m_selectedField);
	}
    else
	{
        if(field != m_clickedPoint)
		{
            m_algorithm->move(m_clickedPoint, field);
		}
        m_clickedPoint = {};
